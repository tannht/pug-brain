...
2. **Memory batch** — Each memory stored via `pugbrain_remember` with proper type, tags, priority
...
1. **Tags** — Extract 2-5 relevant tags from content
   - Use existing brain tags when possible (check via `pugbrain_recall` or `pugbrain_context`)
...
```
pugbrain_recall("PostgreSQL database decision")
```
...
After confirmation, store via `pugbrain_remember`:
