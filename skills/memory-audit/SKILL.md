...
Step 2: pugbrain_health         → purity score, component scores, warnings, recommendations
Step 3: pugbrain_context        → recent memories, freshness indicators
Step 4: pugbrain_conflicts(action="list") → active contradictions
```
...
|-------|--------|----------|
| Active contradictions | `pugbrain_conflicts list` | CRITICAL if >0 |
...
**Approach**: Cross-reference memory tags with current `pugbrain_context` output.
...
**Data source**: `pugbrain_health` provides connectivity, diversity, orphan_rate.
...
Recommendation: Resolve via pugbrain_conflicts
