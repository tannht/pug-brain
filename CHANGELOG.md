...
- **Consolidation post-run hints** — warns about orphan neurons (>20%) and missing consolidation after running `pug consolidate`
...
- **`pug flush` stdin blocking** — Process hangs forever when spawned as subprocess without piped input; `sys.stdin.read()` blocks because no EOF is sent. Added 5s timeout via `ThreadPoolExecutor` (fixes #27)
...
- **`pug remember --stdin`** — CLI now supports piped input for safe shell usage (issue #21)
...
- **Telegram Backup Integration** — Send brain `.db` files to Telegram
  ...
  - CLI: `pug telegram status`, `pug telegram test`, `pug telegram backup [--brain NAME]`
...
- **Export Markdown** — `pug brain export --format markdown -o brain.md`
...
- **Original Timestamp** — `event_at` parameter on `pugbrain_remember`
  ...
  - CLI: `pug remember "Meeting" --timestamp "2026-03-02T08:00:00"`
...
- **FalkorDB Graph Storage Backend** — Optional graph-native storage replacing SQLite for high-performance traversal
  ...
  - Migration CLI: `pug migrate falkordb` to move SQLite brain data to FalkorDB
...
- **MCP Tool Tiers** — Config-based filtering to reduce token overhead per API turn
  ...
  - CLI command: `pug config tier [--show | minimal | standard | full]`
...
- **Config presets** — Three built-in profiles: `safe-cost` (token-efficient), `balanced` (defaults), `max-recall` (maximum retention). CLI: `pug config preset <name> [--list] [--dry-run]`
...
- **CLI `--version` flag** — `pug --version` / `pug -V` now prints version and exits (standard CLI convention)
...
- **`pug install-skills`** — new CLI command to install skills to `~/.claude/skills/`
...
- **`pug init --skip-skills`** — skills are now installed as part of `pug init`; use `--skip-skills` to opt out
...
- **Proactive Brain Intelligence** — 3 features that make the brain self-aware during normal usage
  ...
  - **Expired Memory Hint** — Health pulse detects expired memories via cheap COUNT query on `typed_memories` table. Surfaces hint when count exceeds threshold (default: 10): `"N expired memories found. Consider cleanup via pug list --expired."`
...
- **Habitual Recall** — ENRICH, DREAM, LEARN_HABITS consolidation strategies
  ...
  - `pugbrain_habits` MCP tool, `pug habits` CLI, `pug update` CLI
...
- **CLI `pug index` command**: Index codebase from command line with `--ext`, `--status`, `--json` options
