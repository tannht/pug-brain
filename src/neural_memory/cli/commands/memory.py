...
def remember(
    ...
    Examples:
        pug remember "Fixed auth bug by adding null check"
        pug remember "We decided to use PostgreSQL" --type decision
        pug remember "Need to refactor auth module" --type todo --priority 7
        pug remember "API_KEY=xxx" --redact
        pug remember "Meeting at 8am" --timestamp "2026-03-02T08:00:00"
        echo "content with backticks" | pug remember --stdin --type context
    """
    import sys
    ...
    async def _remember() -> dict[str, Any]:
        ...
        if project and project_id is None:
            return {
                "error": f"Project '{project}' not found. Create it with: pug project create \"{project}\""
            }
...
def recall(
    ...
    Examples:
        pug recall "What did I do with auth?"
        pug recall "meetings with Alice" --depth 2
        pug recall "Why did the build fail?" --show-routing
        pug recall "project status" --min-confidence 0.5
    """
...
def context(
    ...
    Examples:
        pug context
        pug context --limit 5 --json
        pug context --fresh-only
    """
