...
```
pugbrain_stats → total memories, type distribution, age distribution
pugbrain_health → activation efficiency, recall confidence, connectivity
pugbrain_habits(action="list") → learned workflow patterns
```
...
For each of the top 5 tags in the brain:
  1. pugbrain_recall("What do we know about {tag}?", depth=2)
...
4. **Contradictory data?** — Do memories conflict with each other?
   - Fix: Conflict resolution via `pugbrain_conflicts`
